...
## for the small tuning parameter c. It runs the simulation with c = 1.
##########################################################################
## Data: Synthetic data
##########################################################################
...
install.packages("l1ideal_1.4.tar.gz", repos = NULL, type = "source")
if(packageVersion("l1ideal") != "1.4") {
  stop("Please install the correct version of l1ideal package")
  ## Version 1.4 is same as 1.2 except line 14 in l1ideal.c (#define SLICE_W 1)
}
library(l1ideal)
library(posterior)
library(coda)
...
saveRDS(dat, "../../data/simulation/tuning_param/sim_c_small_dat.rds")
saveRDS(res_c, "../../data/simulation/tuning_param/sim_c_small.rds")
